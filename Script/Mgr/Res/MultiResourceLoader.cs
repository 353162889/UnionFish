...
public class MultiResourceLoader
{
    ...
    public void LoadList(List<string> names, Action<MultiResourceLoader> OnComplete = null, Action<Resource> OnProgress = null, ResourceType resType = ResourceType.UnKnow)
    {
        ...
        for (int i = 0; i < names.Count; i++)
        {
            ...
            {
                LH.LogError("Can not has same name in one MultiResourceLoader");
                return;
            }
            else
            {
        ...
        for (int i = 0; i < names.Count; i++)
        {
            ResourceMgr.Instance.GetResource(names[i], OnFinish, OnFinish, resType);
        }
    }
    ...
    private void OnFinish(Resource res)
    {
        ...
        {
            LH.LogError("[MultiResourceLoader] load " + res.path + " fail!");
        }
        _finishCount++;
        List<Action<Resource>> list;
    ...
    public void Clear()
    {
        if (_finishCount < _loadList.Count)
        {
            ...
            {
                ResourceMgr.Instance.RemoveListener(_loadList[i], OnFinish, OnFinish);
            }
        }
        _mapTryGetRes.Clear();
        _finishCount = 0;
