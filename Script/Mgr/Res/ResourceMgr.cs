...
public class ResourceMgr : MonoBehaviour
{
    ...
    public static ResourceMgr Instance
    {
        get
        {
            if (_instance == null)
            {
                _instance = Drive.drive.AddMissingComponent<ResourceMgr>();
            }
            return _instance;
        }
    }
    ...
    public void Init(bool resourceLoadMode)
    {
        ...
        ResourceFileUtil.Instance.Init();
    }
    ...
    public static void ReleaseUnuseResource()
    {
        ResourceMgr.Instance.ReleaseUnUseRes();
    }

    protected void DebugCurRes()
    {
        string str = "";
        foreach (var item in _mapRes)
        {
    ...
    public static void DebugCurrentRes()
    {
        ResourceMgr.Instance.DebugCurRes();
    }

    public static Resource GetResourceInLuaByType(string path,ResourceHandler onSucc,int resType)
    {
        if (string.IsNullOrEmpty(path))
        {
            LH.LogError("[GetResource]ResName can not is null!");
            return null;
        }
        if (Application.isEditor)
        {
            if (path.IndexOf(".") == -1)
            {
                Debug.LogError("路径需要加上后缀.xxx:" + path);
            }
        }
        return ResourceMgr.Instance.GetResource(path, onSucc, null, (ResourceType)resType);
    }

    public static Resource GetResourceInLua(string path,ResourceHandler onSucc)
    {
        return GetResourceInLuaByType(path, onSucc, (int)ResourceType.UnKnow);
    }

    public static void RemoveListenerInLua(string path,ResourceHandler onSucc)
    {
        ResourceMgr.Instance.RemoveListener(path, onSucc);
    }
    ...
    private void OnResourceDone(Resource res)
    {
        ...
        else
        {
            ...
            if (res.refCount > 1)
            {
                LH.LogError("DestroyResource[resPath=" + res.path + "],RefCount>1.");
            }
            else
            { 
    ...
    {
        string resName = AssetBundleMgr.Instance.GetAssetBundleNameByAssetPath(res.path);
        return AssetBundleMgr.Instance.GetDirectDependencies(resName);
    }
    ...
    public string GetCacheResourceKey(string resPath)
    {
        ...
        return AssetBundleMgr.Instance.GetAssetBundleNameByAssetPath(resPath);
    }
