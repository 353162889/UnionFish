...
public class LaunchCompress
{
    ...
    public static void CompressDir(string strDirectory, string zipedFile)
    {
        using (System.IO.FileStream ZipFile = System.IO.File.Create(zipedFile))
        {
    ...
    private static void CompressStep(string strDirectory, ZipOutputStream s, string parentPath)
    {
    ...
    /// <param name="strDirectory">The directory.</param>
    /// <param name="zipedFile">The ziped file.</param>
    public static void CompressDirExcept(string strDirectory, string zipedFile,string exceptExtension)
    {
        using (System.IO.FileStream ZipFile = System.IO.File.Create(zipedFile))
        {
            using (ZipOutputStream s = new ZipOutputStream(ZipFile))
            {
                s.SetLevel(6);
                CompressStepExcept(strDirectory, s, "", exceptExtension);
            }
        }
    }
    ...
    /// <param name="s">The ZipOutputStream Object.</param>
    /// <param name="parentPath">The parent path.</param>
    private static void CompressStepExcept(string strDirectory, ZipOutputStream s, string parentPath, string exceptExtension)
    {
        ...
        foreach (string file in filenames)// 遍历所有的文件和目录
        {
            ...
            if (Directory.Exists(file))// 先当作目录处理如果存在这个目录就递归Copy该目录下面的文件
            {
                ...
                pPath += file.Substring(file.LastIndexOf("\\") + 1);
                pPath += "\\";
                CompressStepExcept(file, s, pPath, exceptExtension);
            }

            else // 否则直接压缩文件
            {
                if(!file.EndsWith(exceptExtension))
                { 
                    //打开压缩文件
                    using (FileStream fs = File.OpenRead(file))
                    {
        ...
        using (ZipInputStream s = new ZipInputStream(stream))
        {
            ...
            while ((theEntry = s.GetNextEntry()) != null)
            {
                ...
                if (pathToZip != "")
                {
                    ...
                    //directoryName = Path.GetDirectoryName(pathToZip).Replace("\\","/") + "/";
                }
